...
- 每日签到奖励：支持经验、物品、补签卡。
- 补签机制：可消耗补签卡补签当前年份内最近一个漏签日期。
- OP 可视化编辑：可在 GUI 中编辑 7 日循环奖励。
- 多物品奖励：单日支持多种物品，支持 NBT 文本。
...
- Java: `17+`（运行游戏）；构建默认使用 `gradle.properties` 中固定的 JDK 21。

## 安装方式

1. 将构建产物 `signin-1.0.3.jar` 放入服务端 `mods`。
2. 客户端也放入同版本 `signin-1.0.3.jar`（建议与服务端一致）。
